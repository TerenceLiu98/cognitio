...
- App 负责工作区、监听、串行队列、状态恢复和 Agent 启动；单一 `$llmwiki` Skill 负责 MinerU 解析、知识写入、校验、commit 和 push。
...
- 首次设置依次完成：语言、工作区、工具检测、Agent/模型、MinerU 模式、GitHub 仓库、托管方式、Skill 安装、端到端预检。
- App 配置以带 `schemaVersion` 的 JSON 原子写入 macOS Application Support；MinerU Token 只进入 Keychain。日志使用本地滚动 JSONL，并对 Token、远程 URL 凭据和敏感环境变量脱敏。
...
- App 在任务开始前要求 Git 工作区干净。Skill 使用 `git pull --ff-only`、内容校验、Quartz 构建、commit、push；提交包含 `LLMWiki-Job: <uuid>` trailer，用于幂等恢复。
...
- Tauri commands 仅暴露设置读写、工作区初始化、预检、暂停/恢复、任务列表、Retry/Cancel、打开目录/站点；Rust 通过事件向 UI 推送任务快照，不让前端直接执行 shell。
- Adapter 根据 CLI 版本生成参数，未知主版本 fail closed。禁止使用跳过 sandbox/permissions 的危险参数；只授予 Wiki 和任务暂存区所需权限。
- App 仅传递最小环境变量集合，包括 `PATH`、`HOME`、临时目录、必要认证、标准代理变量和 MinerU Token；Token 不进入 prompt 或日志。
- App 提供隐藏 CLI：`llmwiki parse --input ... --output ... --mode ... --json`。Skill 调用该命令访问 MinerU，随后读取 Markdown、复用概念、写 Paper/Concept 页面并发布。
...
- 新 Wiki 可由 `gh repo create` 创建，也可克隆已有仓库；`gh` 不可用时允许填写 Git remote。新仓库默认 private，Git/`gh` 认证失败必须在预检阶段明确阻塞。
- Quartz 使用当前官方 Quartz 5，并锁定具体 commit/tag；构建期生成版本化模板包，运行时复制模板，不跟随浮动 `v5` 分支。
- 模板包含 Paper/Concept 目录、BibTeX、`.gitignore`、中立 Markdown 规范、Quartz 配置、插件锁文件和首页布局；本地验证执行 `npm ci`、`npx quartz plugin install`、`npx quartz build`。
- Cloudflare Pages 路径生成官方构建参数并由用户在控制台连接 GitHub；GitHub Pages 路径生成 Node 24 Actions workflow。两种方式都由用户完成平台授权，App 只保存并验证站点 URL。
...
3. **M2 Wiki 初始化**：完成 Quartz 5 模板打包、Git/`gh` 接入、private 默认策略、Cloudflare/GitHub Pages 配置生成和本地 Quartz 构建。
...
- Cloudflare Pages 与 GitHub Pages 均生成配置但不调用其管理 API；私有网站认证、团队协作、多分支并发和自动模板升级不属于 v0.1。
