...
cognitio is a local-first macOS menu bar utility that turns academic PDFs into a linked Quartz knowledge base. It watches a workspace inbox, delegates paper analysis to Codex, Claude Code, or OpenCode, then validates, commits, and pushes the generated Wiki changes before archiving the source PDF.
...
Development uses the separate `Cognitio Dev` product name and `com.cognitio.desktop.dev` bundle identifier. This prevents a development process from intercepting launches of the installed Cognitio app through the single-instance plugin.
...
On first launch, select an empty workspace, choose a site title, agent, and MinerU mode, enter a GitHub repository, then run preflight. Initialization creates `inbox/`, `processing/`, `done/`, `failed/`, and `wiki/`, installs the versioned cognitio skill (currently invoked as `$llmwiki`), pins Quartz to a reviewed commit, and configures the Git remote. GitHub Actions installs Quartz dependencies and builds the site after each push.

Drop PDFs into `<workspace>/inbox/`. Files are queued after three stable observations. cognitio advances up to three jobs concurrently and stores each MinerU result and checksummed parse manifest in its own task directory. Agent execution and app-managed Git publication are serialized for the Wiki. The app exposes Retry, explicit reparse, Cancel, local logs, and a redacted diagnostics archive.
...
The bundled parser is also available as a hidden CLI. The project is named cognitio; its executable currently retains the internal name `llmwiki`:
